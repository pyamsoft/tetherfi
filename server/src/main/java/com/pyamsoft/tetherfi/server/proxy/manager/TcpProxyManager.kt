...
internal constructor(
    ...
    private fun resolveProxyConnectionInfo(connection: Socket): ProxyConnectionInfo? {
        ...
        if (remote !is InetSocketAddress) {
            warnLog { "Block resolve: non-internet socket addresses, we expect clients to be inet: $connection" }
            return null
        }
        ...
        try {
            ...
            e.ifNotCancellation {
                if (e is SocketTimeoutException) {
                    ...
                    errorLog(e) { "Error occurred while establishing TCP Proxy Connection: $proxyConnectionInfo" }
                }
            }
        }
    }
